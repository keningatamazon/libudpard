...
REQUIRE(que.peek()->getPayloadByte(3) == 3);
    ...
    REQUIRE(3 == que.push(&ins.getInstance(), 1'000'000'000'100ULL, meta, 68, payload.data()));  // 68 bytes --> 2 frames
    REQUIRE(4 == que.getSize());
	...
	REQUIRE(q.at(2)->tx_deadline_usec == 1'000'000'000'100ULL);
        REQUIRE(q.at(2)->frame.payload_size == 52); // mtu(= 28 data + 24 header)
        REQUIRE(!q.at(2)->isStartOfTransfer());
        REQUIRE(!q.at(2)->isEndOfTransfer());

	REQUIRE(q.at(3)->tx_deadline_usec == 1'000'000'000'100ULL);
	REQUIRE(q.at(3)->frame.payload_size == 28); // 24 header + 4 crc 
        REQUIRE(!q.at(3)->isStartOfTransfer());
        REQUIRE(q.at(3)->isEndOfTransfer());
    }
...
REQUIRE(-UDPARD_ERROR_OUT_OF_MEMORY == que.push(&ins.getInstance(), 1'000'000'000'200ULL, meta, 1, payload.data()));
    ...
    ti = que.peek();
    REQUIRE(nullptr != ti);
    REQUIRE(ti->frame.payload_size == 52);
    REQUIRE(ti->tx_deadline_usec == 1'000'000'000'100ULL);
    ins.getAllocator().deallocate(que.pop(ti));
    REQUIRE(1 == que.getSize());
    REQUIRE(1 == alloc.getNumAllocatedFragments());
    ...
    meta.priority    = UdpardPriorityLow;
    meta.transfer_id = 25;
    REQUIRE(3 == que.push(&ins.getInstance(), 1'000'000'001'000ULL, meta, 40 + 28, payload.data()));
    REQUIRE(3 == que.getSize());
    REQUIRE(3 == alloc.getNumAllocatedFragments());
    ...
    REQUIRE(nullptr != ti);
    REQUIRE(ti->frame.payload_size == 28);
    REQUIRE(((uint8_t)(CRC68 & 0xFFU)) == reinterpret_cast<const std::uint8_t*>(ti->frame.payload)[24]);
    REQUIRE(((uint8_t)(CRC68 >> 8U)) == reinterpret_cast<const std::uint8_t*>(ti->frame.payload)[25]);
    REQUIRE(((uint8_t)(CRC68 >> 16U)) == reinterpret_cast<const std::uint8_t*>(ti->frame.payload)[26]);
    REQUIRE(((uint8_t)(CRC68 >> 24U)) == reinterpret_cast<const std::uint8_t*>(ti->frame.payload)[27]);
    ins.getAllocator().deallocate(que.pop(ti));
    REQUIRE(0 == que.getSize());
    REQUIRE(0 == alloc.getNumAllocatedFragments());
